...
计数 | O(n)+O(k) | O(n)+O(k) | O(n)+O(k) | O(k) | 不稳定 | 8
> k为待排序数组中的最大值
...
## 八、计数排序
计数排序是一种很特殊的排序，只能用于**取值范围有限的自然数**数组的排序。
算法思想很简单，创建一个统计数组，然后遍历待排序数组，将每个元素出现的次数加起来，存放到统计数组中元素值对应的下标上。

```C++
void Count(vector<int>& v, int begin, int end)
{
	int max = 0;
	for (int n : v)
	{
	...
	vector<int> count(max + 1, 0);	// 如果排序数组中没有重复元素，int可换成bool
	for (int n : v)
		count[n]++;
	int cur = 0;
